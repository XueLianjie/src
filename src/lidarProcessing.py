...
class LidarProcessing:
    ...
    def construct_birds_eye_view(self, data):
        ...
        filter_front_left = np.logical_and(filter_front_left, y_points > -x_points)
        ...
        filter_rear_left = np.logical_and(filter_rear_left, y_points < -x_points)
        filter_rear_left = np.logical_and(filter_rear_left, pixel_vals > 128)
        indices = np.argwhere(filter_rear_left).flatten()

        self.x_near_left = np.mean(x_points[indices])
        self.y_near_left = np.mean(y_points[indices])
        ...
        filter_front_right = np.logical_and(filter_front_right, y_points < x_points)
        ...
        filter_rear_right = np.logical_and(filter_rear_right, y_points > -x_points)
        filter_rear_right = np.logical_and(filter_rear_right, pixel_vals > 128)
        indices = np.argwhere(filter_rear_right).flatten()

        self.x_near_right = np.mean(x_points[indices])
        self.y_near_right = np.mean(y_points[indices])
        ###############
