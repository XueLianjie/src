...
class particleFilter:
    ...
    def updateWeight(self, readings_robot):
        ...
        for particle in self.particles:
            particle_gravimeter_measurement = particle.read_sensor()
            weights[i] *= weight_gaussian_kernel(gravimeter_measurement, particle_gravimeter_measurement, sigma)
            weights[i] += 1.e-300
            i += 1
    ...
    def resampleParticle(self):
        ...
        for i in range(len(self.particles)):
            if self.particles[i].weight > 0.00000:
                p.append(self.particles[i])
                accum += self.particles[i].weight
                accums.append(accum)

        for i in range(self.num_particles):
            sam = bisect.bisect_left(accums, np.random.uniform(0, 1))
            particles_new.append(p[sam])
    ...
    def particleMotionModel(self):
        ...
        offset = [0.1, 0.1]
        for i in range(self.num_particles):
            maze = self.particles[i].maze
            self.particles[i].try_move(offset, maze)
